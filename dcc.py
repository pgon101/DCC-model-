...
# Cargando datos
data = pd.read_stata('C:/Users/pgonr/Downloads/pablo1.dta')
...
def log_likelihood(beta, data):
    ...
    log_likelihood = ((np.where((norm.pdf(s1) * norm.cdf(r1) + norm.pdf(s2) * (1 - norm.cdf(r2)) + norm.pdf(u1) * (norm.cdf(t2) - norm.cdf(t1))) > 0, (norm.pdf(s1) * norm.cdf(r1) + norm.pdf(s2) * (1 - norm.cdf(r2)) + norm.pdf(u1) * (norm.cdf(t2) - norm.cdf(t1))), np.finfo(float).eps)) * tar)
